...
//! - No business concepts — those belong in engine
//! - File paths, serialization formats are contained within
...
///
/// ```ignore
/// let doc: Document<MySettings> = Document::open("settings.json")?;
/// let val = doc.load()?;                    // read from disk
/// doc.update(|s| s.some_field = 42)?;       // load + modify + save
/// ```
...
///
/// ```ignore
/// let tf = TextFile::open("notes.txt")?;
...
impl TextFile {
    ...
    /// Does NOT read the file — just stores the path.
    pub fn open(path: impl Into<std::path::PathBuf>) -> Result<Self> {
